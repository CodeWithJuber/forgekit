...
With routing fixed at its measured 0.62, reaching 90 % total requires the *other* stages
...
ledger is warm, and is not credible cold.** The cache factor dominates, and it *grows*
...
3. **Correctness guard:** a saving only counts if the external verifier passes the output
   ...
   correct" — applied to every stage; a cache hit that gets reverted is a *negative*
   entry).
4. **Report:** per-stage factors with confidence intervals → `reports/cost-eval.md`;
   ...
   caveat attached. Until then the README may say "62 % measured (routing); ~90 %
...
- Injected context (claims, blast radius, checkpoints) *spends* tokens to save tokens;
