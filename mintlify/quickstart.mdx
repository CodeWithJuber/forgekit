...
<Steps>
  <Step title="Emit every tool's config">
    ...
    Codex, Cursor, Gemini, Aider, Copilot, Windsurf, Zed, Continue, and OpenClaw now
    read the **same** rules, each from its own native file. On OpenClaw the MCP server is
    one extra command: `openclaw mcp add forge-cortex --command forge --arg cortex-mcp`.
  </Step>
