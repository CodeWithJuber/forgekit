...
async function routeUniversalCli(argv) {
  ...
  const { loadRegistry, servableBy } = await import("./router/registry.js");
  const json = argv.includes("--json");
  ...
  const words = argv.slice(1).filter((a, i, arr) => !a.startsWith("--") && !VALUED.has(arr[i - 1] ?? ""));
  const sub = ["outcome", "fit", "models", "universal"].includes(words[0]) ? words.shift() : "universal";
  const root = process.cwd();
  if (sub === "models") {
    ...
    if (json) return console.log(JSON.stringify({ sources: reg.sources, fit: fit?.origin ?? null, models: rows }, null, 2));
  ...
  if (sub === "fit") {
    ...
    console.log(`  refit on ${model.provenance.local.outcomes} recorded outcome(s) over ${model.provenance.local.tasks} task(s); wrote .forge/router_model.json`);
    return;
  }
  ...
  if (sub === "outcome") {
    ...
    try {
      ...
      console.log(`  recorded ${row.model} ${row.passed ? "pass" : "fail"} for task ${row.task} (.forge/route_outcomes.jsonl)`);
    } catch (e) {
  ...
  heading(`${BRAND.brand} route universal — ${rec.objective.kind}${rec.target != null ? ` (target ${rec.target.toFixed(2)})` : ""}\n`);
  ...
  console.log(`  ${rec.candidates} candidate model(s), ${rec.cascadesEvaluated} cascade(s) compared · fit: ${rec.fit.origin}`);
  console.log(`  learn from results: \`${BRAND.cli} route outcome "<task>" --model <id> --pass|--fail --cost <usd>\`, then \`${BRAND.cli} route fit\``);
}
