...
_before_ the model edits code, supplying proof-carrying memory, impact foresight, and
enforced guardrails — and a **cross-tool config compiler** that delivers that brain as
native config into every tool at once.
...
- **Memory that persists across sessions and teammates.** Every lesson, fact, and
  verified reuse is _proof-carrying memory (PCM)_ — a claim trusted only once independent
  oracles raise its confidence above a floor.
