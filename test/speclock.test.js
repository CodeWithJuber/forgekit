...
import { referencedSymbols, snapshot, check } from "../src/speclock.js";

const fixture = () => mkdtempSync(join(tmpdir(), "forge-spec-"));

test("referencedSymbols pulls backtick code tokens", () => {
  const syms = referencedSymbols(
    "The `computeTax` function and `Ledger` class.",
  );
