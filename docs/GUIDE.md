...
```bash
cd your-project
forge init                              # once per repo: emit config for the tools it uses
...
`forge init` emits config only for the tools a repo uses (Claude plus the tools it detects,
or an explicit `--tools <list|all>`) and records that set in `.forge/forge.config.json`
(`tools`), so `forge sync` emits the same set afterwards. A repo with no recorded set, or
one initialised with `--tools all`, gets config for **every** supported agent tool — great
for portability, noisy for a repo where only one tool is ever used: `.cursor/`, `.gemini/`,
`.codex/`, `.zed/`, `.aider.conf.yml`, and friends all show up as tracked clutter. `forge
tools` fixes that without dropping anything `sync` emits (`--reset` clears the primary tool
and leaves the recorded `tools` set alone).
...
- `forge tools <name>` — record `<name>` (`claude` · `cursor` · `gemini` · `codex` ·
  ...
  `AGENTS.md` plus the primary tool's own files always stay tracked. When `forge init`
  recorded a tool set without `<name>`, `<name>` joins it first, so the sync emits its
  config (and Forge owns the copies of your integrations it writes there).
...
| `FORGE_AUTOSYNC`                                               | `0` disables the Stop-hook auto-repair of forge's block in AGENTS.md                                                                                                                                                                    |
