...
test("applyBump regenerates the docs site's changelog page in the release commit", () => {
  const dir = makeFixture();
  try {
    fs.writeFileSync(
    ...
    const rel = "mintlify/changelog/overview.mdx";
    fs.mkdirSync(path.join(dir, "mintlify", "changelog"), { recursive: true });
    fs.writeFileSync(
      path.join(dir, rel),
      `---\ntitle: x\n---\n\n{/* forge:render:changelog:begin (generated by \`${BRAND.cli} docs render\` — do not edit) */}\n{/* forge:render:changelog:end */}\n`,
    );
    const changed = applyBump(dir, "0.4.0", "0.5.0", "2026-07-07");
    assert.ok(changed.includes(rel), changed.join(", "));
    const page = fs.readFileSync(path.join(dir, rel), "utf8");
    assert.match(page, /<Update label="v0\.5\.0" description="July 7, 2026"/);
    assert.doesNotMatch(page, /label="Unreleased"/, "the rotated [Unreleased] is now the release");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
