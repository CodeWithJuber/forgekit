...
export default {
  ...
  emit(ctx) {
    ...
    if (existing) {
      try { settings = JSON.parse(existing); }
      catch { return { tool: this.tool, target: '.gemini/settings.json', action: 'skipped', note: 'existing settings.json is invalid JSON — left as-is' }; }
    }
    const context = settings.context || (settings.context = {});
    const current = Array.isArray(context.fileName) ? context.fileName : (context.fileName ? [context.fileName] : []);
    ...
    return { tool: this.tool, target: '.gemini/settings.json', action, note: had ? 'AGENTS.md already in context.fileName' : 'added AGENTS.md to context.fileName' };
  },
};
