---
title: "Quickstart"
description: "Install Forge, emit your tools' config from one source, and run your first pre-action gate — in about 60 seconds."
---
...
<Steps>
  <Step title="Emit your tools' config">
    ```bash
    cd ~/your-project
    forge init                          # Claude + the tools this repo already uses
    forge init --tools claude,cursor    # or name them (--tools all for every tool)
    ```
    This emits `AGENTS.md`, Claude Code's `CLAUDE.md` and `.mcp.json`, and the config of
    each tool the repo already shows (`.cursor/`, `.codex/`, `.gemini/`,
    `.github/copilot-instructions.md`, …) — plus the `.gitattributes` union-merge rule the
    ledger needs. The choice is recorded, so later syncs emit the same set. Codex, Cursor,
    Copilot, Windsurf, Zed, and OpenClaw read the rules from `AGENTS.md` either way; Gemini,
    Aider, and Continue need their own files, which init writes when it detects those tools
    or you name them in `--tools`. An existing `AGENTS.md` is kept: Forge adds its rules in
    a marked block. On OpenClaw the MCP server is one extra command:
