...
anything. The pipeline tags each file with the relation that reached it: `reverse`
(depends on the change), `sibling` (shares a dependency with it) or `forward` (the change
depends on it). `forge impact` alone walks reverse dependents unless you pass
`--all-relations`.
...
<CardGroup cols={3}>
  ...
  <Card title="Blast radius over threshold" icon="explosion">
    the dependents in the impacted set exceed the default ~25-file threshold.
  </Card>
</CardGroup>
